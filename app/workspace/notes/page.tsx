  ...
  return (
    <div className="mx-auto max-w-3xl px-8 py-10">
      <div className="mb-6 flex items-center gap-3">
        <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-[#FFF1E6] text-[#E8703A]">
          <FileText className="h-4 w-4" />
        </div>
        <div>
          <h1 className="font-display text-xl font-bold text-ink-900">AI Docs</h1>
        ...
        <div className="mt-6">
          <div className="mb-3 flex justify-end">
            <button
              ...
              disabled={exporting}
              className="flex items-center gap-2 rounded-full border border-brand-200 bg-brand-50 px-4 py-2 text-xs font-semibold text-brand-700 transition-colors hover:bg-brand-100 disabled:opacity-40"
            >
          ...
          {/* This is the exact region captured into the PDF, styled to look
              like a professionally designed document rather than raw AI text. */}
          <div ref={docRef} className="overflow-hidden rounded-xl2 border border-surface-border bg-white shadow-card">
    ...
    <div className="space-y-1">
      {lines.map((line, i) => {
        ...
        if (trimmed.startsWith("- ") || trimmed.startsWith("* ")) {
          return (
            <div key={i} className="mb-1.5 flex items-start gap-2.5 pl-1">
              ...
              <p
                className="text-[14.5px] leading-relaxed text-ink-700"
        ...
        return (
          <p
            key={i}
            className="mb-2 text-[14.5px] leading-relaxed text-ink-700"
...
function boldify(s: string) {
  return s.replace(/\*\*(.+?)\*\*/g, '<strong class="text-ink-900">$1</strong>');
}
